...
| **Sprint 4** (1 minggu) | Responsive tablet untuk Dashboard, Tracking, Armada Overview (1 codebase, layout adaptif) | ✅ **SELESAI** — Dashboard sudah pakai ResponsiveCenter. Armada Overview: grid layout 2/3 kolom di tablet. Active Users: grid layout 2/3 kolom di tablet. Semua pakai breakpoint `isTablet`/`responsiveValue` dari breakpoints.dart. |
| **Sprint 5** (wajib — dikonfirmasi) | Ritase mobile ringkas + Workshop To-Do & Riwayat Servis mobile + Checklist Major (isi kondisi + foto) mobile | ✅ **SELESAI** — 3 modul baru live di mobile: RitaseInputScreen (input jumlah rit+satuan), WorkshopTodoScreen (centang tugas+foto), ChecklistMajorScreen (kondisi kendaraan 10 item+foto). Routes & menu diarmada home ditambahkan. |
| **Sprint 6** (opsional, evaluasi kebutuhan) | Background location service penuh untuk Mandor Titik | ✅ **SELESAI** — Tracking interval jalan walau app di-minimize. BackgroundLocationService dengan flutter_background_service, foreground notification, auto-cutoff 18:00. Android permissions: FOREGROUND_SERVICE, ACCESS_BACKGROUND_LOCATION, POST_NOTIFICATIONS. |

...
| **Sprint 1** (1-2 minggu) | Kamera-only + watermark visible (koordinat + waktu wajib, nama karyawan opsional) — widget reusable, terapkan ke semua modul foto | Semua foto lapangan ber-watermark, galeri terkunci total |
| **Sprint 2** (1-2 minggu) | Offline read-cache layer + perbaikan gap outbox armada (checklist, ODO) | Semua layar kerja harian bisa dibuka & submit tanpa sinyal sama sekali |
| **Sprint 3** (1 minggu) | Push notification end-to-end (`google-services.json` tinggal dipasang → wiring backend trigger → deep link) | Notifikasi reminder & approval masuk real-time |
